...
export const CVUploadFlow: React.FC<CVUploadFlowProps> = ({
  ...
  const processUpload = async () => {
    ...
    try {
      ...
      // Step 5: Send welcome email using the final email from analysis data
      // This ensures that if email was edited in preview, the updated email is used
      try {
        console.log('📧 Attempting to send welcome email...');
        
        const finalEmail = cvAnalysis?.personalInfo?.email || extractedPersonalInfo.email;
        const finalName = cvAnalysis?.personalInfo?.name || extractedPersonalInfo.name;
        
        console.log('📧 Using final email for welcome:', finalEmail);
